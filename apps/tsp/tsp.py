...
# [START data_model]
def create_data_model(input_db_path, config):
...
def main(input_db_path, config):
    """Entry point of the program."""
    # Instantiate the data problem.
    # [START data]
    data = create_data_model(input_db_path, config)
    # [END data]
...
if __name__ == '__main__':
    ...
    db_path = os.path.join(current_path, 'tests/tsp.db')
    config_path = os.path.join(current_path, 'tests/config.json')

    with open(config_path, 'r') as f:
        config = json.load(f)
    main(input_db_path=db_path, config=config)
# [END program]